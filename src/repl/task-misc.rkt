...
(define (show-task id)
    ...
    (when (task-ready? t)
        (printf "  Ready ~a.\n" (format-date (assert (task-ready-by t)))))
    (when (task-assigned? t)
        (define u (user-display-name (get-user-by-id (assert (task-assigned-to t)))))
        (printf "  Assigned ~a to ~a.\n" (format-date (assert (task-started-by t))) u))
    (when (task-done? t) 
        (printf "  Completed ~a.\n" (format-date (assert (task-done-by t)))))
    (unless (task-ready? t)
        (printf "  Pending..."))
    (unless (task-assigned? t)
        (define rates (get-user-task-evals (assert (me)) t))
        (define priority (get-task-priority t))
        (printf "    priority #~a (~a)\n"
            (add1 (length (filter (lambda ([t : Task]) (if priority (> (cast (get-task-priority t) Nonnegative-Real) priority) #t))
