...
(define (show-task id)
    ...
    (unless (empty? (task-assigned-to t))
        (define ustrs (string-join (map (compose user-display-name get-user-by-id)
                                        (task-assigned-to t))
                                   ", "))
        (printf "  Assigned to ~a.\n" ustrs))
    (when (task-started? t)
        (printf "  Started ~a.\n" (format-date (task-started-by t))))
    (when (task-done? t) 
        (printf "  Completed ~a.\n" (format-date (task-done-by t))))
    (unless (task-ready? t)
        (printf "  Pending..."))
    (unless (task-started? t)
